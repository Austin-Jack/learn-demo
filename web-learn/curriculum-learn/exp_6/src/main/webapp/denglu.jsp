...
<body>
<%
    if (session.getAttribute("user")!=null){  //将session中的user的name转发给结果页
        request.setAttribute("name",((UserBean)request.getSession().getAttribute("user")).getName());
        request.getRequestDispatcher("dengLuResult.jsp").forward(request,response);
    }
%>
<div class="login">
    <form action="${pageContext.request.contextPath}/treatDengLu" method="post" >
        <table>
