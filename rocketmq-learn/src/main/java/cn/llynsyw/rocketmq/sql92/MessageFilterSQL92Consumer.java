 ...
 * 消息消费者
 */
public class MessageFilterSQL92Consumer {

    public static void main(String[] args) {
        DefaultMQPushConsumer consumer = null;
        try {
            // 1. 创建消费者对象并绑定消费者组
            consumer = new DefaultMQPushConsumer("filter-sql92-mq-consumer");
            // 2. 设置 NameServer 地址
            consumer.setNamesrvAddr("localhost:9876");
            // 3. 订阅消费消息的主题
            // CONSUMED_BUT_FILTERED
            consumer.subscribe("filter-sql92", MessageSelector.bySql("age>=18 AND name='laoliu'"));
            // 4. 设置并创建消息监听器
            consumer.setMessageListener(new MessageListenerConcurrently() {
                ...
                public ConsumeConcurrentlyStatus consumeMessage(List<MessageExt> list, ConsumeConcurrentlyContext ctx) {
                    ...
                    for (MessageExt messageExt : list) {
                        ...
                        System.out.println("属性对象：" + properties);
                    }
